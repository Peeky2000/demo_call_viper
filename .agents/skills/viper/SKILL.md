...
| Pha mỗi vòng | **V + I bắt buộc**; P/E/R theo mục tiêu vòng | **V + I bắt buộc**; P/E/R chỉ ở vòng khai trong `_PROPOSAL.md` |
...
| **P** Polish & Publish | `$viper-polish` → `$viper-publish` → `$viper-dogfood` | Vòng không khai `P` → bỏ qua cả pha · `P1` prod-ready xanh trừ mục `(sau deploy)` → deploy được · `P2` 4 nhóm xanh đủ · production sống · đã thử rollback |
| **E** Evaluate | `$viper-evaluate` | Vòng không khai `E` → bỏ qua · sổ EXPERIMENTS của vòng có số liệu thật (vòng 1: `EXPERIMENTS.md`; vòng N ≥2: `EXPERIMENTS-v<N>.md`) |
| **R / đóng vòng** | `$viper-repeat` | Vòng khai R: go/pivot/kill ghi vào `DECISIONS.md`. Workflow còn archive/bookkeeping và mở vòng mới cho vòng không khai R; không bịa quyết định chỉ để đóng sổ. `scripts/repeat.py --go` không reset gì — legacy vòng trước là hợp đồng (`VIPER.md §1.2`) |
...
Ba lớp guard (`VIPER.md §3c`): `guard_ask.py` chặn lời gọi có cấu trúc `request_user_input` ngoài pha V (luật hành vi trong `AGENTS.md` vẫn canh câu hỏi văn bản) · `guard_bc.py` chặn deploy khi còn nợ tương thích ngược · `guard_ds.py` chặn patch prototype/DESIGN-SYSTEM lệch design system đã chốt.
