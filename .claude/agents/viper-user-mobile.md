...
mcpServers:
  browser:
    command: npm
    args: ["exec", "-y", "--", "@playwright/mcp@latest", "--isolated", "--ignore-https-errors", "--viewport-size", "390,844"]
  mobile:
    command: npm
...
**Cách làm việc**
- Experience web: duyệt bằng skill `viper-browse`. Frontmatter đã đặt sẵn viewport **390×844** (điện thoại phổ biến); đổi sang **360×640** bằng `browser_resize` nếu có chỗ nghi ngờ.
